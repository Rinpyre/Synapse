...
\begin{itemize}
    \item \textbf{Frontend} (\texttt{/frontend}): A frontend that provides the user interface for viewing logs, inputting queries, and displaying AI-generated responses.
    \item \textbf{Backend} (\texttt{/backend}): A php framework that serves as the REST API, handling user requests, processing queries, and interacting with the AI models and databases.
    \item \textbf{AI Service} (\texttt{/ai}): A separate service responsible for managing interactions with the LLMs, processing log data, and generating responses based on user queries. \\
    ...
    \item \textbf{Database} (\texttt{/database}): An SQL Server instance for storing any necessary relational data, and optionally unstructured database instance for saving query results if that feature is implemented.
