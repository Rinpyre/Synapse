...
\fancyhead[R]{\includegraphics[height=0.45cm]{./images/speedadmin-logo.png}}
\fancyfoot[C]{\thepage}
...
\renewcommand{\cftsecnumwidth}{1.25em}
...
SpeedAdmin is a leading provider of specialised software solutions tailored for music and art schools. Their cutting-edge platform streamlines administrative processes and facilitates seamless communication for the schools, enhancing efficiency and effectiveness. Their logs on students, teachers, lectures, instruments, etc. are verbose in nature, making them often difficult to understand for their support and customers' needs alike. We are looking to implement a system to help them improve their overview and the usefulness of own logs to the customer and support teams, leveraging the possibilities offered by AI prompting, and machine learning.

\section{Aim}
Build a containerized AI-powered system that enables support staff and customers to query SpeedAdmin resource logs in natural language, delivering summaries, anomaly detection, and pattern analysis to improve log comprehension and operational insights.
...
\end{itemize}
\textbf{Note:} Objectives marked with an asterisk are considered core objectives that we aim to achieve, while the others are additional features that we will implement if time allows.
...
\section{Methods}
To achieve the project objectives, we will make use of the following methods and technologies:
\begin{itemize}
    ...
    \item \textit{Project Management Tool:} \textbf{\textit{TODO: Add our chosen project management tool (e.g., Trello, Jira) if we decide to use one.}}
          % TODO: Add our chosen project management tool (e.g., Trello, Jira) if we decide to use one.
...
\end{itemize}
\textbf{Note:} \textit{We have deviated from the original suggested tech stack by opting for Laravel instead of ASP.NET since our team has more experience with that framework.}

\section{Architecture}
The system will be designed using a microservice architecture, with the following components:
\begin{itemize}
    \item \textbf{Frontend} (\texttt{/frontend}): A React application that provides the user interface for viewing logs, inputting queries, and displaying AI-generated responses.
    \item \textbf{Backend} (\texttt{/backend}): A Laravel application that serves as the REST API, handling user requests, processing queries, and interacting with the AI models and databases.
    \item \textbf{AI Service} (\texttt{/ai}): A separate service responsible for managing interactions with the LLMs via Ollama, processing log data, and generating responses based on user queries. \\
    ...
    \item \textbf{Database} (\texttt{/database}): A Microsoft SQL Server instance for storing any necessary relational data, and optionally a MongoDB instance for saving query results if that feature is implemented.
